...
export async function collectCommits(daysAgoOverride?: number): Promise<CollectCommitsResult> {
  ...
  appendLog(logs, 'Coleta iniciada.');

  for (let repoIndex = 0; repoIndex < shellConfig.repositories.length; repoIndex += 1) {
    const target = shellConfig.repositories[repoIndex];
    appendLog(logs, `[${repoIndex + 1}/${shellConfig.repositories.length}] Coletando branches de ${target.repo}...`);
    ...
    if (!branches.length) {
      appendLog(logs, `Nenhuma branch encontrada para ${target.repo}.`);
      continue;
    }

    appendLog(logs, `${target.repo}: ${branches.length} branch(es) encontradas.`);
    ...
    for (let branchIndex = 0; branchIndex < branches.length; branchIndex += 1) {
      ...
      for (let authorIndex = 0; authorIndex < shellConfig.authorEmails.length; authorIndex += 1) {
        ...
        if (currentCommits.length > 0) {
          appendLog(logs, `${target.repo} | ${branchName} | ${authorEmail}: ${currentCommits.length} commit(s).`);
        }
